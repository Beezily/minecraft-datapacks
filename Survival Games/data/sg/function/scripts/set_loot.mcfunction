...
execute align xyz positioned ~0.5 ~0.5 ~0.5 as @e[type=minecraft:marker,tag=sgChest,limit=1,sort=nearest,distance=..1] run scoreboard players operation @s sg = %iteration sg
...
# THIS IS NO LONGER NECESSARY IF WE JUST REPLACE ALL OLD MARKERS THE MOMENT THEY SPAWN ^^^ 